...
const getWeather = () => {
  loading.value = true;
  http
    .get(`/weather/3d?location=${location}&key=${key}`)
    .then((res: any) => {
      wd.value = res.daily[0];
      upTime.value = _.dateFormater("yyyy-MM-DD HH:mm:ss", res.updateTime);
      qiIcon.value = ["qi-" + wd.value.iconDay, "qi-" + wd.value.iconNight];
    })
    .catch((err) => {
      console.error(err);
    })
    ...
    <template v-else>
      <div class="flex">
        ...
        <div class="flex-grow">
          <div class="text-base">
            今天白天 {{ wd.textDay
            }} <i class="text-base" :class="qiIcon[0]"></i>，夜晚 {{ wd.textNight }} <i class="text-base" :class="qiIcon[1]"></i>，最高气温
          ...
          <div class="my-1 text-sm text-gray-500">
            风向 {{ wd.windDirDay }}，风力 {{ wd.windScaleDay }} 级，风速 {{ wd.windSpeedDay }} km/h
          </div>
