...
const user_info = ref<UserInfo>(us.info);
watch(
  () => us.info,
  (nv, ob) => {
    user_info.value = nv;
    ...
    <themeComponent>
      <n-page-header>
        <n-grid :cols="5">
          ...
          <n-gi>
            <n-statistic label="注册" value="122" />
          </n-gi>
          <n-gi>
            <n-statistic label="待办" value="36 次" />
          </n-gi>
          <n-gi>
            <n-statistic label="话题" value="83 个" />
          </n-gi>
          <n-gi>
            <n-statistic label="接口调用" value="2,346 K" />
          </n-gi>
        </n-grid>
        <template #title>
          <h3 class="font-semibold">
            早上好！{{
              user_info.nickName || "亲爱的用户"
            }}，开始您一天的苦逼工作吧！
          </h3>
        </template>
        <template #avatar>
          <n-avatar size="large" round src="/atom.svg" />
        </template>
        <template #extra>
          <n-space>
